...
//    int ordinal = 0;
    ...
    // Now fetching the feeds.
    client = new DefaultHttpClient();
    HttpGet get= new HttpGet(gDataService.getFeed());

    get.addHeader("Authorization", "GoogleLogin");
    get.addHeader("auth", authToken);

    for (Header currHeader : get.getAllHeaders()) {
      System.out.println(currHeader);
    }
    response = client.execute(get);
    System.out.println("Response for the feed : \n" + EntityUtils.toString(response.getEntity()));
  }
}
