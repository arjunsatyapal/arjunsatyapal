...
public class CitiesUpdaterPresenter extends Presenter<CitiesUpdaterDisplay> {
  
  public static String PLACE = "cityupdate";
  
  HashMap<Integer, ClientCityData> cityList =
    new HashMap<Integer, ClientCityData>();
  
  public CitiesUpdaterPresenter(final String params,
    ...
    getDisplay().setOnUpdateCitiesClickCallback(new SimpleCallback<Object>() {
      ...
      public void goBack(Object dummy) {
        ...
        String xmlToSend;
        /*
         * Create the XMl to send via any of the two following calls (but not
         * both!)
         */
        ...
        /*
         * ...and then pick one of the two following sentences to send the data
         * to the server
         */
  ...
  void displayCities(String xmlCities) {
    ...
    if (!xmlCities.isEmpty()) {
      ...
      for (int i = 0; i < cities.getLength(); i++) {
        ...
        final Element state =
          (Element) city.getElementsByTagName("state").item(0);
  ...
  void getCitiesViaProxy() {
    ...
    xhrProxy.getFromUrl(realUrl, realPath, params, new AsyncCallback<String>() {
  ...
  void getCitiesViaRequestBuilder() {
    ...
    final RequestBuilder rb =
      new RequestBuilder(RequestBuilder.GET, URL.encode(baseUrl
        + ":80/bookphp/getcities1.php?city=" + getDisplay().getCityNameStart()));
  ...
  void sendCitiesToServerViaRequestBuilder(String xmlToSend) {
    ...
    String baseUrl = "http:" + GWT.getHostPageBaseURL().split(":")[1];
    final RequestBuilder rb =
      new RequestBuilder(RequestBuilder.POST, URL.encode(baseUrl
        + ":80/bookphp/setc2.php?" + "xmldata=" + xmlToSend));
