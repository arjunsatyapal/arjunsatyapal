...
import com.arjunsatyapal.practice.gwt1.client.rpc.WorldServiceAsync;
...
public class Model {
  ...
  public void getCities_nocache(final String country, final String state,
   ...
   */
  static LinkedHashMap<String, LinkedHashMap<String, ClientCityData>> citiesCache =
    new LinkedHashMap<String, LinkedHashMap<String, ClientCityData>>();
  
  public void getCities_withCache_withoutPrefetching(final String country,
    final String state, final int pStart, final int pCount,
    final AsyncCallback<LinkedHashMap<String, ClientCityData>> cb) {
    /*
     * If we call "loadCities(...)" without having selected a country and
     * region, it won't do anything, and just exit gracefully.
    ...
    if (!country.isEmpty() && !state.isEmpty()) {
      ...
      if (!citiesCache.containsKey(country + ":" + state + ":" + pStart)) {
        getRemoteWorldService().getCities(country, state, pStart, pCount,
          new AsyncCallback<LinkedHashMap<String, ClientCityData>>() {
            public void onFailure(final Throwable caught) {
              Window.alert("Failure getting cities: " + caught.getMessage());
            ...
            public void onSuccess(
              ...
              citiesCache.put(country + ":" + state + ":" + pStart, result);
              cb.onSuccess(result);
            }
          });
      } else {
         ...
         */
        cb.onSuccess(citiesCache.get(country + ":" + state + ":" + pStart));
  ...
  public void getCities(final String country, final String state,
    ...
    if (!country.isEmpty() && !state.isEmpty()) {
      ...
      if (!citiesCache.containsKey(country + ":" + state + ":"
        ...
        getRemoteWorldService().getCities(country, state, pStart + pCount,
          pCount, new AsyncCallback<LinkedHashMap<String, ClientCityData>>() {
            public void onFailure(final Throwable caught) {
              // ...error...
            }
            ...
            public void onSuccess(
              ...
              citiesCache.put(country + ":" + state + ":" + (pStart + pCount),
                result);
            }
          });
      }
  ...
  public void
    getCountries(final AsyncCallback<LinkedHashMap<String, String>> cb) {
  ...
  public void getStates_nocache(final String country,
    final AsyncCallback<LinkedHashMap<String, String>> cb) {
