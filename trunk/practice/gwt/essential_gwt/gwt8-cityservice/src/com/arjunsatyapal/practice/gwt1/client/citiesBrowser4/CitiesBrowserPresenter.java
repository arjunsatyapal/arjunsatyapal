...
import com.arjunsatyapal.practice.gwt1.client.countryState.CountryStatePresenter;
...
public class CitiesBrowserPresenter extends Presenter<CitiesBrowserDisplay> {
  ...
  private class DeferredCitiesDisplay implements IncrementalCommand {
    ...
    public boolean execute() {
       ...
       */
      boolean someMore =
        originalCountry.equals(getDisplay().getCountryState().getCountry())
          && originalState.equals(getDisplay().getCountryState().getState()) //
          && originalStart == currentStart;
  ...
  public CitiesBrowserPresenter(final String params,
    ...
    csp =
      new CountryStatePresenter("", getDisplay().getCountryState(), environment);
  ...
  void getAndDisplayCities() {
    ...
    getEnvironment().getModel().getCities(
      ...
      new SimpleCallback<LinkedHashMap<String, ClientCityData>>() {
        @Override
        public void goBack(final LinkedHashMap<String, ClientCityData> result) {
          
          /*
           * Run, as soon as possible, a routine that will display the cities a
           * few at a time.
           */
          DeferredCommand.addCommand(new DeferredCitiesDisplay(result));
        }
      });
