Custom DataSource
-----------------
This example illustrates setting up a DataSource accessing a servlet front
controller (for example struts or Spring MVC controller) for the various
DataSource operations. Once the request is received by your controller, you can
implement the logic to act upon it - for example delegating the request to your
service API facade or DAO. This sample uses Spring MVC controllers.
...
- 'ant war'
  Compile for deployment and bundle into CustomDS.war file.

  If you have a web container operating on localhost:8080, you can deploy
  CustomDS.war into the web container and access it at:

    http://localhost:8080/CustomDS/index.html
