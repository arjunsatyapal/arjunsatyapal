...
var dbName = "<% out.write(dbName); %>"
var tableName = "<% out.write(tableNamesCSV); %>"
var className = "<% out.write(classNamesCSV); %>"
var puName = "<% out.write(puName); %>"
var outputFormat = "<% out.write(outputFormat); %>"
var overwrite = "<% out.write(overwrite); %>"
var dumpTestData = "<% out.write(dumpTestData); %>"
var timestampsAsDates = "<% out.write(timestampsAsDates); %>"
var returnSQLType = "<% out.write(returnSQLType); %>"
isc.Window.create({
    ID: "isc_generatorWindow",
    title: "Batch DataSource Generator",
    autoCenter: true, width: 600, height: 440, 
    items: [
...
if (classNames != null) {
    for (int i = 0; i < classNames.length; i++) {
        ...
        Map dsConfig = DataSourceTools.getDataSourceConfigFromHibernateMapping(classNames[i]);
        if (dsConfig == null) {
            try {
                dsConfig = JPADSGenerator.getDataSourceConfigFromJPAClass(classNames[i]);
                fromJPAMapping = true;
            } catch (Exception ex) {}
            if (dsConfig == null) {
                fromMapping = false;
